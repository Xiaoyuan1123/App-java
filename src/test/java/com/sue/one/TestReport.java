...
public class TestReport extends TestListenerAdapter{
	...
	public void onStart(final ITestContext context){
		File htmlReportDir = new File("test-output/customizeHtml-report");//run之后文件夹下面不会自动HTML文件
		//File htmlReportDir = new File("C:/Users/hyuan/javaworkspace1.8/App-java/test-output/customizeHtml-report");
		if (!htmlReportDir.exists()) {
			htmlReportDir.mkdirs();
			}
		String reportName = formateDate()+".html"; 
		//SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		//String reportName = sdf.format(new Date())+".html";  
